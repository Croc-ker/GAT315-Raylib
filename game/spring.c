...
void ApplySpringForce(ncSpring_t* springs) {
	for (ncSpring_t* spring = springs; spring; spring = spring->next) {
		...
		ApplyForce(spring->body1, Vector2Scale(ndirection, force), FORCE);
		ApplyForce(spring->body2, Vector2Scale(Vector2Negate(ndirection), force), FORCE); //could also use -force instead
	}
}